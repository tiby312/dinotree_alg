...
impl IsPoint for Bot{
    type Num=f32n;
    fn get_center(&self)->[f32n;2]{
        [f32n!(self.pos[0]),f32n!(self.pos[1])]
    }
}
...
impl KnearestEveryDemo{
    pub fn new(dim:[f32n;2])->KnearestEveryDemo{
...
impl DemoSys for KnearestEveryDemo{
    fn step(&mut self,_cursor:[f32;2],c:&piston_window::Context,g:&mut piston_window::G2d,_check_naive:bool){
...

        {
            ...
            let mut tree=DinoTree::new(axgeom::YAXISS,(),&bots,|b|{
                Conv::from_rect(aabb_from_pointf32(b.pos,[0.0;2]))
            });
            ...
            for a in tree.iter(){
                ...
                let r=5.0;
                let r=Conv::from_rect(aabb_from_pointf32(p,[r;2]));
                draw_rect_f32n([0.0,1.0,0.0,0.5],&r,c,g);
            } 
            /*
