...
mod ray_isize{
    ...
    impl<'a,'c:'a> RayTrait for RayT<'a,'c>{
        type T=BBox<i32,Bot2>;
        type N=i32;

 

        fn intersects_rect(&self,rect:&axgeom::Rect<Self::N>)->bool{
            match ray_intersects_box_int(&self.ray,rect){
        ...
        fn compute_distance_bot(&mut self,a:&Self::T)->Option<Self::N>{
            draw_rect_i32([1.0,0.0,0.0,0.8],a.get(),self.c,self.g);
            match ray_intersects_box_int(&self.ray,a.get()){
...
pub struct RaycastDemo{
    tree:DinoTree<axgeom::XAXISS,BBox<i32,Bot2>>,
    counter:f32,
    dim:Rect<i32>
}
impl RaycastDemo{
    pub fn new(dim:Rect<F32n>)->RaycastDemo{
...
impl DemoSys for RaycastDemo{
    fn step(&mut self,cursor:Vec2<F32n>,c:&piston_window::Context,g:&mut piston_window::G2d,check_naive:bool){
        ...
        let k={
            ...
            let res1=raycast::raycast(&tree,self.dim,ray_isize::RayT{ray,c:&c,g,height});
            match res1{
                Some((mut bots,dis))=>{
                    ...
                    if check_naive{
                        ...
                        if bots.len()!=bots2.len(){
                            ...
                            for (&v,&p) in bots.iter().zip(bots2.iter()){
                                ...
                                {
                                    let a=v as *const BBox<i32,Bot2>;
                                    let b=p as *const BBox<i32,Bot2>;
                                    //println!("{:?}",(a,b));
                                    if  a!=b {
                                        println!("{:?}\n{:?}\n{:?}",v,p,ray.point);
                                        panic!();
        ...
        
        {
            ...
            impl<'a,'b:'a> dinotree_alg::graphics::DividerDrawer for Bla<'a,'b>{
                type N=i32;
                fn draw_divider<A:axgeom::AxisTrait>(&mut self,axis:A,div:i32,cont:[i32;2],length:[i32;2],depth:usize){
